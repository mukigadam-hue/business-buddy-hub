...
-- Update search_businesses to support business type filter and return currency symbol
DROP FUNCTION IF EXISTS public.search_businesses(text, integer, integer, text, text);

CREATE OR REPLACE FUNCTION public.search_businesses(
  _query text DEFAULT ''::text,
  _limit integer DEFAULT 20,
  _offset integer DEFAULT 0,
  _country_code text DEFAULT ''::text,
  _district text DEFAULT ''::text,
  _business_type text DEFAULT ''::text
)
RETURNS TABLE(
  id uuid,
  name text,
...
AS $function$
  SELECT b.id, b.name, b.business_type, b.address, b.contact, b.email, b.logo_url,
         b.business_code, b.products_description, b.country_code, b.district,
         b.currency_symbol, b.last_active_at
  ...
  WHERE 
    ...
    AND (_business_type = '' OR b.business_type = _business_type)
  ORDER BY 
